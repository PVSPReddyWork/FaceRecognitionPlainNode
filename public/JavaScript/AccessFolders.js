...
async function GetFilesFromFolders(
  folderPath,
  folderName = '',
  isAccessingProjFiles = false
) {
  try {
    const FILES_PATH = isAccessingProjFiles
    ...
    await fs.readdirSync(FILES_PATH).forEach(async (file) => {
      if (!file.toString().includes('.')) {
        ...
        const filePath = isAccessingProjFiles
          ? path.join(__dirname, folderPath + '/' + file.toString())
          : folderPath + '/' + file.toString();
        const imageObject = {
...
const GetImagesFromFolders = async (filePath, fromProjectFolders) => {
  try {
    ...
    const allFiles = fromProjectFolders
      ? await AccessProjectFiles(filePath)
      : await AccessLocalFiles(filePath);
      ...
      allFiles.forEach(async (eachFile) => {
        ...
        eachFile.filesList.forEach(async (element) => {
          if (element.fileURL.includes('.')) {
            var fileType =
              element.fileURL.split('.')[element.fileURL.split('.').length - 1];
            if (
